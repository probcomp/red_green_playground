...
function DiameterTrialByTrialPage() {
  ...
  // Load metrics from CSV on component mount and when diameter changes
  useEffect(() => {
    const loadMetrics = async () => {
      try {
        ...
        const response = await fetch(`/metrics_csv?diameter=${diameter}`);
        ...
        {trials.map((trialName) => {
          ...
          const metricValue =
            selectedMetric && metricsData && metricsData[selectedMetric]
              ...
              : null;
          const isValidMetricValue = metricValue !== null && metricValue !== undefined && !Number.isNaN(metricValue);
