...
function App() {
  ...
  const [timestepsPerFrame, setTimestepsPerFrame] = useState(5);
  const [timestep, setTimestep] = useState(0.02);
  const [fps, setFps] = useState(40); // Default FPS
  ...
  const renderArrowForTarget = (target) => {
    ...
    const handleArrowDragStop = (e, d) => {
      ...
      const newAngle = Math.atan2(deltaY, deltaX); // Calculate the new angle in radians
      const snappedAngle = Math.round((newAngle * 180) / Math.PI); // Snap to the nearest degree
      const snappedLength = Math.round(Math.sqrt(deltaX ** 2 + deltaY ** 2) / px_scale / interval) * interval;
  
      updateTargetDirectionAndSpeed(snappedAngle, snappedLength); // given in degrees
      // Update the entity's direction and speed
      const updatedEntity = {
        ...target,
        direction: (snappedAngle * Math.PI) / 180, // given in radians
        speed: snappedLength,
      };
...
// const KeyPressAtCounter = () => {
//   const [counter, setCounter] = useState(0); // Counter updated every second
//   const [keyStream, setKeyStream] = useState([]); // Stores key press data
//   const [currentKey, setCurrentKey] = useState(null); // Tracks the most recent key press

//   useEffect(() => {
//     // Function to handle keydown events
//     const handleKeyPress = (event) => {
//       setCurrentKey(event.key); // Track the latest key being pressed
//     };
...
//       // At the exact moment of the counter increment, record the key press if any
//       if (currentKey) {
//         const utcTimeMs = Date.now(); // UTC time in milliseconds
//         setKeyStream((prevStream) => [
//           ...prevStream,
//           { counter: counter + 1, key: currentKey, utcTimeMs },
//         ]);
//       }
...
//       {/* Counter Display */}
//       <div style={{ width: '100px', textAlign: 'center', borderRight: '1px solid black', padding: '10px' }}>
//         <h2>Counter</h2>
...
//         <h1>Key Press Stream at Counter Increment</h1>
//         <div
//           style={{
//             height: '300px',
//             overflowY: 'auto',
...
//         >
//           {keyStream.length > 0 ? (
//             keyStream.map((item, index) => (
//               <div key={index}>
//                 Counter: {item.counter}, UTC: {item.utcTimeMs} - You pressed: "{item.key}"
//               </div>
//             ))
//           ) : (
...
// export default KeyPressAtCounter;
